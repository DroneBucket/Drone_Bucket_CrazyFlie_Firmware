...
static struct crtpLinkOperations radiolinkOp = {
		.setEnable = radiolinkSetEnable, .sendPacket = radiolinkSendCRTPPacket,
...
/* Get the RSSI value */
void radiolinkGetRSSI(uint8_t* rssi_value){
	*rssi_value = rssi;
}
...
LOG_GROUP_START(radio) LOG_ADD(LOG_UINT8, rssi, &rssi)
